 ...
 * Updated-On : $Date: 2012/10/04 14:07:01 $
 *
 * Purpose:
...
public class LineChart {
	...
	public void createReport(String graphType, String chartSrc,
			...
			ArrayList<String> aListItems, boolean isImageGeneration,String strLHS, String strRHS, String color,int lineThickness)
		...
		try {
			...
			ApplicationConstants.APP_LOGGER
					.info(" Line Chart init method finished ");
			...
			ApplicationConstants.APP_LOGGER.info(" Error occur Line Chart  "
					+ e.getMessage());
		...
		try {
			...
			if (isImageGeneration) {
				...
				reportDesignHandle.saveAs(ApplicationConstants.REPORT_FOLDER
						...
						+ "linechart.rptdesign");
				rptdesign = ApplicationConstants.REPORT_FOLDER + Path.SEPARATOR
						+ strTimeStamp + graphType + "linechart.rptdesign";
	...
	private void createMasterPages(String headerTitle,String color)
		...
		try {
			...
			if (getPageNumber() == 0)
			{
				TextItemHandle textItem3 = reportDesignHandle.getElementFactory().newTextItem("test5");
...
//					+ Path.SEPARATOR + ApplicationConstants.OOYALA_IMAGE_LINE);
			...
			log.error("error in bar chart" + e.getMessage());
		}
	...
	protected ExtendedItemHandle createMulitYSeriesChart(String graphType,
		...
		// format Specifier
		/*
		 * NumberFormatSpecifier nf=NumberFormatSpecifierImpl.create(); 2
		...
		//bs1.getLineAttributes().setThickness(getLineThickness());
		bs1.getLineAttributes().setThickness(8);
