 ...
 * Updated-On : $Date: 2012/10/31 08:37:05 $
 *
 * Purpose:
...
public class MultipleBarChart {
	
	private ReportDesignHandle reportDesignHandle			 = null;
	private ElementFactory elementFactory							= null;
	private String dataSetName 												= "datasetmultibar"+System.currentTimeMillis();
	static Logger log 																= Logger.getLogger(MultipleBarChart.class.getName());
...
//					+ Path.SEPARATOR + ApplicationConstants.OOYALA_IMAGE_LINE);
	...
	protected ExtendedItemHandle createMulitYSeriesChart(String graphType,
		...
		sdX.getSeriesPalette().update(ColorDefinitionImpl.create(0,162,210)); // SET THE COLORS IN THE PALETTE
	...
	if(hashMap.get("yaxis7")!=null){
			...
			BarSeries bs7 = (BarSeries) BarSeriesImpl.create();
	...
	if(hashMap.get("yaxis8")!=null){
			...
			BarSeries bs8 = (BarSeries) BarSeriesImpl.create();
	...
	if(hashMap.get("yaxis9")!=null){
		...
		BarSeries bs9 = (BarSeries) BarSeriesImpl.create();
